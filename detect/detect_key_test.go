...
func TestDetectKeyFailNilBytes(t *testing.T) {
	...
	for testNumber, test := range tests {
		...
		_, _, err := DetectKey( []byte(test.Bytes) )
		if nil == err {
		...
		if expected, actual := errNilBytes, err; expected != actual {
			t.Errorf("For test #%d, expected (%T) %q, but actually got (%T) %q. {%#v}", testNumber, expected, expected, actual, actual, test.Bytes)
			continue
		}
...
func TestDetectKeyFailEmptyRequest(t *testing.T) {
	...
	for testNumber, test := range tests {
		...
		if expected, actual := errEmptyRequest, err; expected != actual {
			t.Errorf("For test #%d, expected (%T) %q, but actually got (%T) %q. {%#v}", testNumber, expected, expected, actual, actual, test.Bytes)
			continue
		}
...
func TestDetectKeyFailNotKey(t *testing.T) {
	...
	tests := []struct{
		...
		{
			Bytes: []byte("\v\"apple\".\"banana\".\"cherry\""),
		},
	...
	for testNumber, test := range tests {
		...
		if _, ok := err.(notKeyComplainer); !ok {
			t.Errorf("For test #%d, expected error to be a Not Key Complainer, but actually got (%T) %q. {%#v} {%s}", testNumber, err, err, test.Bytes, string(test.Bytes))
			continue
		}
...
func TestDetectKey(t *testing.T) {
	...
	tests := []struct{
		...
		{
			Bytes: []byte(`"apple"."banana"="something is here"`),
			ExpectedBegin: 0,
			ExpectedEnd:                   16,
			Expected:     `"apple"."banana"`,
		},
		{
			Bytes: []byte(`"apple"."banana"."cherry"="something is here"`),
			ExpectedBegin: 0,
			ExpectedEnd:                            25,
			Expected:     `"apple"."banana"."cherry"`,
		},
		...
		{
			Bytes: []byte(`"apple banana"."\"cherry\""."\"grape\".\"kiwi\""."=watermelon".""="something is here"`),
			ExpectedBegin: 0,
			ExpectedEnd:                                                                    65,
			Expected:     `"apple banana"."\"cherry\""."\"grape\".\"kiwi\""."=watermelon".""`,
		},
		...
		{
			Bytes: []byte(`"one"."two"."three"."four"."five"="something is here"`),
			ExpectedBegin: 0,
			ExpectedEnd:                                    33,
	...
	for testNumber, test := range tests {
		...
		actualBegin, actualEnd, err := DetectKey( []byte(test.Bytes) )
		if nil != err {
		...
		if expected, actual := test.ExpectedEnd, actualEnd; expected != actual {
			t.Errorf("For test #%d, expected \"end\" index to be %d {%s}, but actually got %d {%s}.", testNumber, expected, string(test.Bytes[test.ExpectedBegin:test.ExpectedEnd]), actual, string(test.Bytes[actualBegin:actualEnd]))
			continue
		}
